        ...
        return new Promise((resolve, reject) => {
            request(payload, function(err, response, body) {
                if (err) {
                    ...
                    console.log(body)
                    console.log("-----------------------------------");
                    reject(new Error(`Response code ${response.statusCode} received from OVH on recording put`));
                } else {
                    resolve();
                }
            });
...
exports.make_container = function (access, feed, callback) {
    ...
    request(payload, function (err, response, body) {
        if (err) {
            ...
            callback(`
            Response code $ { response.statusCode } received from OVH on container put `);
        } else {
            callback(null);
        }
    });
