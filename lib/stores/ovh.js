...
class OVHStore {
    constructor() {
        checker('Mongo data source', ['BACKUP_OVH_ENDPOINT', 'BACKUP_OVH_APP_KEY',
            'BACKUP_OVH_APP_SECRET', 'BACKUP_OVH_CONSUMER_KEY', 'BACKUP_OVH_PROJECT'
        ])
    ...
    async _connect() {
        ...
        for (const s of services) {
            if (s === process.env.BACKUP_OVH_PROJECT) {
                this.service = s;
            }
        }
        if (!this.service) {
            debug(`Failed to acquire service - ${process.env.BACKUP_OVH_PROJECT}`)
            return;
        }
