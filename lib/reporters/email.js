...
class EmailReporter {
    ...
    async send (report) {
		...
		debug(JSON.stringify(report, null, 2));
		const mailOptions = this.make_mail_options('scott.frees@gmail.com', `${process.env.BACKUP_INSTANCE_NAME} backup SUCCESS`, report, success);
	...
	async sendError (report) {
		debug(JSON.stringify(report, null, 2));
		const mailOptions = this.make_mail_options('scott.frees@gmail.com', `${process.env.BACKUP_INSTANCE_NAME} backup FAILED`, report, failure);
